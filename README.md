...
開発時にすぐログインできるよう、`admin@example.com / password` のユーザーと複数のプロジェクト/タグ/直近数週間分のエントリを投入するシードスクリプトを用意しています。
...
既に同じユーザーが存在する場合は上書きせず、紐づくデモデータのみ作り直されます。何度でも実行して最新のテストデータを生成できます。
